...
The order matters. Terrain texturing and boundary reconciliation must install
...
## Lesson 5: Lower The Terrain

Open `Scripts/DeepWaterTexturing.cs` and `Scripts/DeepWaterFloorHeight.cs`.

DFU terrain generation uses heightmaps and tile classification jobs. The mod
wraps DFU's texturing system and inserts its own lowering work before tile
textures are assigned.

The important flow:

1. `MarkDeepWaterJob` decides which samples can be lowered.
2. `LowerMarkedHeightsJob` lowers them with deterministic Perlin variation.
3. DFU assigns tile textures.
4. `ConvertFloorTilesJob` changes pure water floor tiles to dirt.

This is why the seabed has shape and texture while the actual visible water is
drawn separately.

Pay attention to `DeepWaterFloorHeight.ComputeLoweredHeight()`. It has a
matching inline copy in the Burst job. If you change one, change the other.

## Lesson 6: Repair Tile Edges

Open `Scripts/BoundaryReconciler.cs`.

Terrain tiles are generated independently. At a shared edge, two tiles can
store different heights for the same world coordinate. That creates visible
seams or vertical spikes.

`BoundaryReconciler` listens for promoted terrain and reconciles each loaded
neighbor. Its rule is deliberately simple:

- land stays land;
- if either inside sample is land, preserve the edge at ocean height;
- otherwise lower both sides to the same deterministic water height.

The result is stable no matter which tile loads first.

## Lesson 7: Draw The Water Surface

Open `Scripts/WaterSurfaceManager.cs`, `Scripts/WaterSurfaceResources.cs`, and
`Shaders/StenciledWaterSurface.shader`.

The terrain lowering creates space for water, but it does not draw water. The
...
## Lesson 8: Make Outdoor Swimming Work
...
## Lesson 9: Add Underwater Presentation

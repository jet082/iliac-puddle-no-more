...
namespace DeepWaters
{
    internal static class UnderwaterLootPlacement
    {
        ...
        private const float LootFloorLift = 0.3f;
        ...
        public static bool ResolveSeafloorAt(float worldX, float worldZ, out float seafloorWorldY, out Transform terrainTransform)
        {
            ...
            Vector3 worldPos;
            if (!DeepWaterWorld.TryResolveSeafloorPosition(worldX, worldZ, SeafloorYClearance, LootFloorLift, out worldPos, out terrainTransform))
                return false;

            seafloorWorldY = worldPos.y;
            return true;
        }

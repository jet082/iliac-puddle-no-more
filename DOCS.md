...
1. Carve holes in vanilla terrain over ocean-connected water cells and replace them with a procedural seafloor sub-mesh up to 250m deep.
2. Draw a transparent water surface above the new seabed.
...
- `DeepWaterTileData.cs`: per-`DaggerfallTerrain` cache. Holds climate index,
  ocean-connectivity verdict, and a distance-to-coast field computed via a
  chamfer-distance pass.
...
- `WaterDepth`: maximum ocean depth in meters (5–250). Scales the bathymetry
  function's output proportionally so the same seafloor shape can be presented
  at any depth from "puddle" to "abyss." Ocean climate reaches near this value;
  swamp coasts stay much shallower regardless.
...
- seafloor local/world Y — comes from `DeepBathymetry` when the tile has a
  `DeepWaterTileData` with `IsOceanConnected = true`, otherwise falls back to
  the vanilla heightmap value;
- ocean local/world Y;
- terrain transform parent;
- column depth.

Because seafloor Y now routes through the bathymetry function, every consumer
(enemies, fish, loot, decorations, swim driver) sees the procedural seafloor
without needing to know about the sub-mesh. The function is the source of
truth; the sub-mesh is its visual + collision projection.
...
The mod no longer modifies the vanilla heightmap. Instead it carves holes
through vanilla terrain mesh in ocean-connected water cells and replaces them
with a procedural sub-mesh whose vertices come from the bathymetry function.

### Bathymetry function

`DeepBathymetry.SampleDepthMeters(worldX, worldZ, climateIndex, distanceToCoast)`
is a pure deterministic function returning depth-below-ocean in meters. It
composes:

1. **Climate base depth.** Ocean = 210m, Subtropical = 175m, Rainforest = 150m,
   Swamp = 28m, Woodlands/Haunted = 165m, Mountain variants = 185m, Desert = 90m.
   Anything not listed falls through to the open-ocean profile.
2. **Coast-distance shelf.** A smoothstep ramp from `ShelfMinDepth = 4m` at the
   shoreline to the climate base depth at `ShelfRampMeters = 90m` offshore.
   Keeps the sub-mesh tight under vanilla shore tiles so there's no visible
   void at the coast.
3. **Macro Perlin** (~4200m period, ±38% of climate base): bay-scale "deep
   regions vs shallow regions" variation, anchored to world coords so adjacent
   tiles agree at shared edges.
4. **Mid Perlin** (~330m period, ±22m): ridges, hummocks, seamounts.
5. **High Perlin** (~24m period, ±4m): micro-roughness so the seabed isn't a
   smooth plane up close.
6. **Trench mask.** A thresholded noise field with smooth ramping. Above the
   threshold, adds up to 95m of extra depth. Trenches only form past
   `TrenchMinDistanceToCoast = 200m` so coasts don't get undercut.

The final value is scaled by the user's `WaterDepth` setting as a fraction of
`MaxAbsoluteDepth = 250m`, then clamped. Adjacent tile sub-meshes that sample
this at the same world coords automatically agree at shared edges — no seam
reconciliation is needed.

### Per-tile data

`DeepWaterTileData` is a `MonoBehaviour` attached to each `DaggerfallTerrain` at
promotion time. It stores:

- the climate index resolved via
  `DaggerfallUnity.Instance.ContentReader.MapFileReader.GetClimateIndex`;
- `IsOceanConnected` — a heuristic checking whether at least one heightmap edge
  has a contiguous water run of at least 25% of the edge length. Pure-ocean and
  coastal tiles pass; inland lakes and narrow rivers fail;
- `distanceMeters[,]` — a per-heightmap-sample distance-to-nearest-land field
  computed via a two-pass chamfer-distance approximation. Bilinearly
  interpolated for arbitrary world XZ queries.

### Hole mask and sub-mesh

`DeepWaterFloorBuilder` subscribes to `DaggerfallTerrain.OnPromoteTerrainData`.
Per promotion it:

1. Ensures a `DeepWaterTileData` is attached and initialized for the tile.
2. If `IsOceanConnected` is false, removes any prior floor mesh, clears holes,
   and returns. Inland water and dry tiles stay vanilla.
3. Otherwise computes a hole mask at `terrainData.holesResolution`. A cell is
   holed when:
   - all four corner heightmap samples are at or below ocean threshold; AND
   - the cell center is at least `HoleBufferMeters = 12m` from the nearest
     land sample.
   The buffer leaves the marching-squares shore-transition tiles unholed so the
   beach gradient still renders. With cell width ~6.4m, the buffer is roughly
   two cells deep.
4. Applies the hole mask via `TerrainData.SetHoles` and spawns a
   `DeepWaterFloorMesh` child component.

### Sub-mesh construction

`DeepWaterFloorMesh.Build` generates a regular 65x65 vertex grid covering the
tile footprint. For each vertex:

- world XZ resolved from local XZ + terrain origin;
- distance-to-coast queried from `DeepWaterTileData`;
- depth from `DeepBathymetry.SampleDepthMeters`;
- local Y set to `oceanLocalY - depth`;
- UV set to `(worldX, worldZ) / 24m` for tiled texture grain;
- vertex color packed as `(depthBand, climateBand, distanceBand, 1)`.
...
`DeepWaters/Seafloor` blends three colors — shallow sand, mid-depth silt,
deep rock — based on the vertex-color depth band, with a coast-distance
"shelf boost" that pushes harbor floors toward sand. Inland-water climates
(low climate band) lerp toward a muddy palette. The shader is opaque, receives
DFU fog, and uses Lambert lighting with a slight upward tilt so the
ambient-from-above feel survives down in trenches.
...
- Seafloor sub-meshes generate once per promotion from a pure depth function.
- Adjacent tiles agree at shared edges by sampling at world coords; no seam
  reconciliation is needed.
